...
public class StmtVisitor implements StmtSwitch {
	...
	private Insn buildArrayPutInsn(ArrayRef destRef, Value source) {
		...
		if(array.getType() instanceof RefType)
		{
			arrayType = ArrayType.v(array.getType(), 1);
		}
		else
		{
			arrayType = (ArrayType) array.getType();
		}
		
		String arrayTypeDescriptor = SootToDexUtils.getArrayTypeDescriptor(arrayType);
